...
fn to_table(type_name: String, results: &[Vec<SingleRunStats>]) -> Vec<String> {
    let name_width = (results.iter().flatten().map(|result| result.name.len()).max().unwrap_or(28) + 2).max(30);
...
fn to_table_background(type_name: String, results: &[(String, TxnStats)]) -> Vec<String> {
    let name_width = (results.iter().map(|(name, _)| name.len()).max().unwrap_or(28) + 2).max(30);
